    ...
    <div class="row justify-content-start">
        <div class="col-md-6">
            ...
            <form action="{{ route('income.store') }}" method="post" enctype="multipart/form-data">
                ...
                <input class="form-control mb-3 date" type="number" name="date" value="1" min="0" max="30" required>
                ...
                @foreach($income as $item)
                    <tr class="h-100">
                        ...
                        <td class="align-middle d-flex align-items-center h-100 py-3">
                            ...
                            <form action="{{ route('storePayment') }}" method="post" class="d-inline">
                                @csrf
                                <input type="hidden" name="name" value="Príjem: {{ $item->name }}">
                                <input type="hidden" name="price" value="+{{ $item->income }}">
                                <button type="submit" class="btn-success btn-sm ms-1">
